...
# Create a share-safe project archive for BBJF App.
# Excludes secrets, git history, dependencies, build output, local Supabase state,
# logs, backups, exports, and previously generated zip files.
...
ARCHIVE_NAME="bbjf-app-safe-${STAMP}.zip"
ARCHIVE_PATH="${EXPORT_DIR%/}/${ARCHIVE_NAME}"
TMP_DIR="$(mktemp -d)"
PROJECT_DIR="$TMP_DIR/bbjf-app"
...
(
  cd "$TMP_DIR"
  zip -qr "$ROOT_DIR/$ARCHIVE_PATH" bbjf-app
)
